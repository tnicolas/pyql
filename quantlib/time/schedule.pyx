...
from quantlib cimport ql # _schedule, _date, _calendar
...
cdef public enum Rule:
    ...
    ThirdWednesday = ql.ThirdWednesday
...
cdef class Schedule:
    ...
    def __init__(self, Date effective_date, Date termination_date,
            ...
            int business_day_convention=ql.Following,
            int termination_date_convention=ql.Following,
           int date_generation_rule=Forward, end_of_month=False):

        self._thisptr = new ql.Schedule(
            deref(effective_date._thisptr.get()),
            deref(termination_date._thisptr.get()),
            deref(tenor._thisptr.get()),
            deref(calendar._thisptr),
            <ql.BusinessDayConvention>business_day_convention,
            <ql.BusinessDayConvention>termination_date_convention,
            <ql.Rule>date_generation_rule, end_of_month
        )
    ...
    def at(self, int index):
        cdef ql.Date date = self._thisptr.at(index)
