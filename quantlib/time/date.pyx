            ...
            if(isinstance(tenor, str)):
                try:
                    ...
                    raise ValueError("Parse Error: could not parse period %s" \
                                     % tenor)
                self._thisptr = \
                new shared_ptr[ql.Period](new ql.Period(<Integer> int(t),
                                         <ql.TimeUnit> _TU_DICT[u]))
            else:
                self._thisptr = new shared_ptr[ql.Period](
                    new ql.Period(<ql.Frequency>args[0])
                )
        elif len(args) == 2:
            self._thisptr = new shared_ptr[ql.Period](
                new ql.Period(<Integer> args[0], <ql.TimeUnit> args[1])
            )
        elif len(args) == 0:
            self._thisptr = new shared_ptr[ql.Period]()
        else:
